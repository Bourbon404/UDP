...
function getLandByLngLat(lng,lat){
	var lLatBand=0;
	//lat的绝对值
	while(Math.abs(lat)>ER_LATBAND_ARRAY[lLatBand].lMaxLatitude){
		lLatBand++;
	}
	...
	RndchinaPlugin.RequestPlugin(json,function(data){
		...
		switch (data.ResCode) {
			case "0":
				...
				var land=getLandByPoint(parseFloat(array[0]),parseFloat(array[2]));
				if(land!=undefined){
...
function getCurrentBuildings(array){
	...
	RndchinaPlugin.RequestPlugin(json,function(data){
		data=JSON.parse(data);
		if(data.CmdType=="3"&&(data.SubType=="-95"||data.SubType=="161")){
			//data为返回的数据
			switch (data.ResCode) {
				case "0":
					if(data.LandNum!="0"){
